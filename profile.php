        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col lg:flex-row gap-8 mb-16">
                ...
                <div class="w-full lg:w-1/2 flex flex-col justify-center">
                    ...
                    <p class="text-slate-500 text-lg leading-relaxed">Welcome back, <strong class="text-brand-900"><?=htmlspecialchars($currentUser['name'])?></strong>. Track your reading progress, manage bookmarks, and explore your history.</p>
                </div>
                ...
                <div class="w-full lg:w-1/2 flex flex-col sm:flex-row gap-6">
                    ...
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex-1">
                        ...
                        <div class="mb-5">
                            ...
                            <div class="text-right mt-1 text-xs font-bold text-emerald-600">
                                <?=$goal ? round($progress) . '%' : 'Set a goal to track progress'?>
                            </div>
                        </div>
                        
                        <form method="post" action="actions.php" class="flex gap-2">
                            <input type="hidden" name="action" value="update_goal">
                            <input type="number" name="goal" class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-accent-500 focus:border-accent-500 outline-none text-sm transition-all" placeholder="Annual goal" value="<?=$goal?>" min="1" required>
                            <button type="submit" class="px-4 py-2 bg-accent-500 hover:bg-accent-600 text-white font-bold rounded-xl text-sm transition-colors shadow-md">Update</button>
                        </form>
                    </div>
                            ...
                            <div class="flex flex-col bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
                                ...
                                <div class="p-5 flex flex-col flex-grow">
                                    ...
                                    <div class="flex gap-2 mt-auto">
                                        <button onclick="toggleBookmark(<?=$book['id']?>, this)" class="w-full py-2.5 rounded-xl border border-slate-200 text-rose-500 hover:bg-rose-50 hover:border-rose-200 font-bold text-sm transition-colors flex items-center justify-center">
            ...
            <div class="mb-8">
                <div class="flex items-center mb-8">
                    <i class="bi bi-clock-history text-2xl text-accent-500 mr-3"></i>
                    <h3 class="text-2xl font-extrabold font-heading text-brand-900 m-0">Recently Accessed</h3>
                </div>
                ...
                <?php if ($history): ?>
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
                        <?php foreach ($history as $book): ?>
                            <div class="flex flex-col bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
                                <div class="relative pt-[130%] bg-slate-100 overflow-hidden cursor-pointer" onclick="openQuickView(this)" data-book-id="<?=$book['id']?>" data-title="<?=htmlspecialchars($book['title'])?>" data-author="<?=htmlspecialchars($book['author'])?>" data-description="<?=htmlspecialchars($book['description'])?>" data-cover="<?=htmlspecialchars($book['cover'])?>" data-category="<?=htmlspecialchars($book['category'])?>" data-drive-link="<?=htmlspecialchars($book['drive_link'])?>">
                                    <span class="absolute top-3 right-3 z-10 bg-slate-800/80 backdrop-blur-sm text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">
                                        <?=htmlspecialchars($book['category'])?>
                                    </span>
                                    <img src="<?=htmlspecialchars($book['cover'])?>" class="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 opacity-90">
                                </div>
                                <div class="p-5 flex flex-col flex-grow">
                                    <h4 class="font-bold font-heading text-lg text-slate-700 mb-1 truncate"><?=htmlspecialchars($book['title'])?></h4>
                                    <p class="text-sm text-slate-500 mb-4">By <?=htmlspecialchars($book['author'])?></p>
                                    <a href="download.php?id=<?=$book['id']?>" target="_blank" class="mt-auto w-full py-2.5 rounded-xl border border-brand-900 text-brand-900 hover:bg-brand-900 hover:text-white font-bold text-sm transition-colors flex items-center justify-center text-decoration-none">
                                        Access Again <i class="bi bi-arrow-right-short ml-1 text-lg"></i>
                                    </a>
                                </div>
                            </div>
                        <?php endforeach; ?>
                ...
                <?php else: ?>
                    <div class="text-center py-12 bg-slate-100 rounded-2xl border border-dashed border-slate-300">
                        <i class="bi bi-clock-history text-slate-300 text-5xl mb-3 block"></i>
                        <h4 class="font-bold font-heading text-slate-600">No Recent Activity</h4>
                        <p class="text-slate-500 text-sm">Your reading history will appear here once you open resources.</p>
                    </div>
                <?php endif; ?>
            </div>
                ...
                <form method="post" action="actions.php">
                    <div class="modal-body p-6">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-accent-500 focus:border-accent-500 outline-none transition-all" name="name" value="<?=htmlspecialchars($currentUser['name'])?>" required>
                        </div>
                        <div class="mb-4">
                            <label class="block text-sm font-bold text-slate-700 mb-2">Email Address</label>
                            <input type="email" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-accent-500 focus:border-accent-500 outline-none transition-all" name="email" value="<?=htmlspecialchars($currentUser['email'])?>" required>
                        </div>
                        <div class="mb-2">
                            ...
                            <input type="password" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-accent-500 focus:border-accent-500 outline-none transition-all" name="password" placeholder="Leave blank to keep current">
                        </div>
                    </div>
                    <div class="modal-footer bg-brand-50 border-t border-slate-100 p-4">
                        ...
                        <button type="submit" class="px-5 py-2 bg-accent-500 hover:bg-accent-600 text-white rounded-xl font-bold transition-colors shadow-md">Save Changes</button>
                    </div>
                </form>
            </div>
        </div>
